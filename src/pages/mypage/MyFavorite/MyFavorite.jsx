...
function MyFavorite(props) {
      ...
      .then((response) => {
        ...
        axios
          .get("/api/saving/1", {
            headers: {
              Authorization:
                "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJJRDEiLCJpYXQiOjE3MTQ3MTkyMzQsImV4cCI6MTcxNTkyODgzNH0.EzlCW0O0ZYlLZl3iRCOoaBCEhqzs1-Pfme1iqCtqNaw",
            },
          })
          .then((response) => {
            if (response.status !== 200) {
  ...
  const showRules = (savingId) => {
    navigate(`/rules`, { state: { savingId: savingId } });
  };
          ...
          {showTerms ? (
            <>
              <div className="join-container">
                  ...
                  <div className="info-details">
                    ...
                    <ul>
                      <p>
                        ...
                        <div>
                          <div>
                            실명의 개인 (1인 1계좌) *기존 스마트폰 적금 포함
                          </div>
                          <div>
                            (단, 우리 WON 더 스테이지 응모는 만 12세 이상부터
                            가능)
                          </div>
                        </div>
                      </p>
                ...
                <img
                  className="info-detailimg"
                  src={terms}
                  alt="terms"
                  width={500}
                  height={400}
                />
                <button className="join-button" onClick={onClickJoin}>
                  가입하기
                </button>
              </div>
            </>
            ...
            <>
              <div className="last-container">
                <p className="savinglist-titletext">
                  우리 <span className="savinglist-bluetext">WON</span> 최애
                  적금
                </p>
                ...
                <button className="last-ctabutton" onClick={onClickComplete}>
                  완료
                </button>
              </div>
            </>
            ...
            <>
                ...
                {savingList.map((saving, index) => (
                  <div key={index} className="savinglist-container">
                    <div className="savinglist-title">
                      <div>
                        <p className="savinglist-titletext">
                          우리 <span className="savinglist-bluetext">WON</span>{" "}
                          최애 통장
                        </p>
                        <p className="savinglist-account">
                          {saving.savingAccount}
                        </p>
                      </div>
                      <p className="savinglist-celebrity">
                        {saving.celebrityName}
                      </p>
                    </div>
                    ...
                    <div className="savinglist-buttoncontainer">
                      <button onClick={() => showRules(saving.savingId)}>
                        규칙
                      </button>
              ...
              <button className="savinglist-ctabutton" onClick={onClickOpened}>
                최애 통장 개설하기
              </button>
            </>
          )}
